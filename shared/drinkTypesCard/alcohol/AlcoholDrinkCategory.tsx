			...
			<div data-route={checkRoute} className={styles.container}>
				{checkRoute
					? AlcoholicDrinkData.map(({ id, bglight, bgdark, icon, text }: any) => {
							return <DrinkTypeCard key={id} id={id} icon={icon} bg={theme === 'light' ? bglight : bgdark} text={text} cardType='categories' />;
					  })
					: AlcoholicDrinkData.slice(0, 4).map(({ id, bglight, bgdark, icon, text }: any) => {
							return <DrinkTypeCard key={id} id={id} icon={icon} bg={theme === 'light' ? bglight : bgdark} text={text} />;
					  })}
			</div>
