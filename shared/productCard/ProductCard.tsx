...
const ProductCard = ({ data }: Props) => {
	const { themeColor, setCart, drinkType } = useGlobalContext();
	...
	const handleSvgColor = () => {
		...
		// return route !== "/alcohol" ? softColors[themeColor] : alcoholColors[themeColor];
	...
	return (
		<div data-type={themeColor} className={styles.cardContainer}>
			<Link href={`/productDetails/${formatURL(data.productName)}`}>
				<div
					className={styles.clip_path}
					onMouseEnter={() => setIsHovered(true)}
					onMouseLeave={() => setIsHovered(false)}
					data-hover={isHovered}
				>
					<svg
						viewBox="-10 0 350 250"
						fill=""
			...
			<div>
				<h3 className={styles.productName}>{data && data.productName}</h3>
			</div>
			<div>
				<p className={styles.productPrice}>NGN{formatNum(data.productPrice)}</p>
			</div>
			<Button buttonType="primary" className={styles.btn} onClick={addToCart}>
				<h4 data-type={themeColor} className={styles.buttonText}>
					Add to Cart
				</h4>
			</Button>
		</div>
