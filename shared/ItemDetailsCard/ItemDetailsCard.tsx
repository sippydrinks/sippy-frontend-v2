...
const ItemDetailsCard = (data: CartDetailsCardProps) => {
	const [isHover, setIsHover] = useState<boolean>(false);
	const { theme, setCart, cart, drinkType } = useGlobalContext();
	...
	const handleDelete = () => {
		if (itemIndex !== -1) {
			const updatedState = localCart.filter((cartItem: any) => cartItem.id !== localCart[itemIndex].id);
	...
	return (
		<div data-quantity={data.productQuantity} data-theme={theme} data-type={data.cardType} className={`${styles.card_container} ${data.className}`}>
			<div className={styles.product_details}>
				<div className={styles.productImage_container} onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)} data-hover={isHover}>
					<svg xmlns='http://www.w3.org/2000/svg' width='125' height='101' viewBox='0 0 125 101' fill='none'>
						<use href='svgPath_ID' stroke={theme === 'dark' ? '#FFF' : '#000'} strokeWidth={4} className={styles.svg_stroke} />
						<path
			...
			<div data-type={data.cardType} className={`${styles.counter_container} ${data.actionBtnClass}`}>
				...
				<div data-theme={theme} onClick={handleDelete} className={styles.delete_icon_container}>
					<div className={styles.delete_icon}>
						<Image alt='delete' src={`/svgs/delete-icon-${theme}.svg`} fill />
					</div>
				</div>
			</div>
			...
			{!data.productQuantity && (
				<div className={styles.outOfStock_container}>
					<div className={styles.outOfStock}>
						<Image alt='out-of-stock' fill priority src='/svgs/OutOfStock.svg' />
					</div>
				</div>
			)}
		</div>
